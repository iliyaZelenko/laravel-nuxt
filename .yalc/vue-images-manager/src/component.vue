<template>
  <div class="IZ-file-uploader">
    ...
    <template v-if="dataForUpload">
      <div
        v-if="percentCompleted"
        class="IZ-file-uploader__preview"
      >

        <div
          class="IZ-file-uploader__percent-completed"
          slot="progress"
        >
        ...
        <div
          v-if="!hidePreview"
          class="IZ-file-uploader__preview-gallery"
        >
          <template v-if="dataForUpload.length >= 0">
            ...
            <div
              v-for="(file, key) in dataForUpload"
              :key="key"
              class="IZ-file-uploader__preview-gallery-item"
            >
              <div class="IZ-file-uploader__preview-gallery-item-thumbail">
                <!-- :ref="`file${key}` -->
                <img
                  :src="setPreview(file, key)"
                  :alt="file.name"
                  ref="file" >
              </div>
          ...
          <template v-else>
            ...
            <div>
              <div class="IZ-file-uploader__preview-gallery-item-thumbail">
                <img
                  :src="setPreview(dataForUpload)"
                  :alt="dataForUpload.name"
                  ref="file"
                >
              </div>
              <div class="IZ-file-uploader__preview-gallery-item-info">
    ...
    <div class="IZ-file-uploader__slot-label">
...
<script>
import { formatBytes, haveNormalTotalSize, haveNormalSize, sizeStrToBytesConvert } from './helpers.js' // , sizeConvert

// window.console.log(sizeStrToBytesConvert('1MB'))
// window.console.log(sizeConvert(1, 'MB', 'B'))

// window.console.log(sizeConvert(1, 'KB', 'B'), 1048576 * 1)

export default {
  name: 'ImagesManager', // vue component name
  props: {
    accept: {
      type: String,
    ...
    fileMaxSize: {
      type: String,
      'default': '1MB' // sizeConvert(1, 'MB', 'B')
    },
    filesTotalMaxSize: {
      type: String,
      'default': '10MB'
  ...
  methods: {
    ...
    async onFileChange (e) {
      ...
      if (this.dataForUpload) {
        const haveNormalSizeInfo = haveNormalSize(this.dataForUpload, this.fileMaxSizeBytes)
        if (!haveNormalSizeInfo.ok) {
          // const { msg, errorFiles, errorFilesNames } = haveNormalSizeInfo
          this.$emit('error', haveNormalSizeInfo)
          this.resetForm()
          return
          // this.$emit('error', {
          //   msg,
        ...
        const haveNormalTotalSizeInfo = haveNormalTotalSize(this.dataForUpload, this.filesTotalMaxSizeBytes)
        if (!haveNormalTotalSizeInfo.ok) {
          this.$emit('error', haveNormalTotalSizeInfo)
          this.resetForm()
          return
        }
        // ---------------------------------------------------------------------
        const eventData = {
          ...
          totalSize: haveNormalTotalSizeInfo.sum
        }
  ...
  computed: {
    fileMaxSizeBytes () {
      return sizeStrToBytesConvert(this.fileMaxSize)
    },
    filesTotalMaxSizeBytes () {
      return sizeStrToBytesConvert(this.filesTotalMaxSize)
    }
  }
}
</script>
